...
# ╔═╡ 180073fd-f1f1-4c05-81e8-aa04b1a6eea9
begin
	using Plots
	using LinearAlgebra
	using JuMP
...
# ╔═╡ 7dde41c0-6c39-4d9e-9a1b-20e1d45a1606
begin
...
md"""
- Tutorial repo on my Github page: [https://github.com/lrsantos11/Tutorial-Julia-Opt](https://github.com/lrsantos11/Tutorial-Julia-Opt)
"""
...
* Packages from [JuMP-dev](https://www.jump.dev/)
    
     * [JuMP](https://jump.dev/JuMP.jl/v0.19.0/index.html): algebraic modeling language for linear, quadratic, and non-linear optimization (with or without constraints)
        
* Some solvers available
     * [HiGHS](https://github.com/jump-dev/HiGHS.jl) for linear programming (LP) problems, convex quadratic programming (QP) problems, and mixed integer programming (MIP) problems.  (open-source)
     * [Ipopt](https://github.com/jump-dev/Ipopt.jl) for nonlinear optimization problems (open-source)
...
* Let's install:
     *  `JuMP`: Julia defautl modeling package
...
# ╔═╡ 7f71465a-c202-4155-a6a3-9e59bad6f84d
md"""
...
# ╔═╡ 6651c25e-e92e-43bf-be25-95b623404a09
md"""
##
* Consider problem
```math
...
# ╔═╡ 331a92ca-e581-4a61-941d-d66f74ea80ba
begin
    function plot_feasible()
        contour(range(-0.5, 10, length=100), range(-0.5, 5, length=100), 
            ...
            frame_style=:origin)
        plot!([0;  0 ; 8; 6  ; 0 ],
          [ 4; 0 ; 0; 1.5 ; 4], c=:blue, lw=2, lab="Feasible set",series=:path)
    end
    plt1 = plot_feasible()
end

# ╔═╡ 2ab3569e-f659-4e9d-a8cc-47d71e3d6428
md"""
##
"""

# ╔═╡ dad80261-e59f-44fa-bff1-947f2cc09d66
begin
	plot_feasible()
	scatter!([6],[1.5],label="Solution")
end

# ╔═╡ 02347472-b98a-4e53-ba4a-291f38e9cf06
md"""
##
"""

# ╔═╡ 6ae61ef8-d468-44e3-ab90-ccc61191cc9a
let
     # Model and Solver
	...
	# lpmodel = Model(HiGHS.Optimizer)
     ...
     # Variables, bounds and type
    	@variable(lpmodel, x ≥ 0)
		@variable(lpmodel, y ≥ 0)

     # Constraints
    	@constraint(lpmodel, 3x + 4y ≤ 24)
	    @constraint(lpmodel, 5x + 12y ≤ 48)

     # objective function
		@objective(lpmodel, Min, -12x - 20y)
	# Print Model
	print(lpmodel)
     # Solver call
    optimize!(lpmodel)
...
# ╔═╡ 7546dd3b-2f8f-43bf-bfac-392f4ebcfa2a
md"""
## Let's take a look at each step of the code"""

# ╔═╡ 237cefd4-1d0c-4975-8cd8-02dc485017c6
md"""
...
# ╔═╡ 155e97ea-01b8-416a-84bc-e321b73e4500
md"""
...
# ╔═╡ 3f582153-57fe-4d7f-b645-e1560f056a7c
md"""
...
# ╔═╡ 41d4e8d2-4a1e-4aab-8bd3-96b92a7ec1a2
md"""
...
# ╔═╡ a5073221-451e-49f1-a42f-2462d3451725
md"""
...
- `reverse` _"transpose"_ in the inverse way
...
[diag(reverse(M),i) for i in -2:2]

# ╔═╡ fb0b7226-6503-4cf3-9f15-92bf38d67a79
md"""
...
# ╔═╡ c784dac9-830b-44e9-b679-67b82ba00205
md"""
$(LocalResource("img/n_queens4.png"))
"""

# ╔═╡ e69df317-a3ed-482d-8311-3486df4789e0
@bind N Slider(4:32, show_value=true)

# ╔═╡ cc29c8ef-ab67-4d2d-bc2c-864f96d92b22
begin
	NQueens = Model(HiGHS.Optimizer)
	# set_silent(NQueens)

    
    #let's create an N x N chessboard of binary values. 0 will represent an empty 		space on the board and 1 will represent a space occupied by one of our queens
    @variable(NQueens, queen[1:N,1:N], Bin)

	#There must be exactly one queen in a given row/column

	# Constraints on the sum of rows (for loop version)
	for i in 1:N
		@constraint(NQueens, sum(queen[i,:]) .== 1)
	end
	# Constraints on sum of columns (list compreehension)
	@constraint(NQueens, [sum(queen[:,j]) for j=1:N] .== 1)
	
	
	# Main diagonal constraints
	@constraint(NQueens,[sum(diag(queen,i)) for i = -(N-1):(N-1)] .<= 1)
	
	# Secondary diagonal constraints
	@constraint(NQueens,[sum(diag(reverse(queen,dims=1),i)) for i = -(N-1):(N-1)] .<= 1)
	
	optimize!(NQueens)
	print(NQueens)	
end

# ╔═╡ a2114a56-6fcd-4679-b44d-36f1e075f6e8
queen

# ╔═╡ 4f664a38-d17c-4bf9-b9b4-288838648ba8
solution = round.(Int, value.(queen))

# ╔═╡ 6567060a-4d98-493a-a007-b5c09f346559
let
	NQueens = Model(HiGHS.Optimizer)
	set_silent(NQueens)
	N = 4
...
# ╔═╡ 3be447f3-385f-4891-9074-fc95d183b324
md"""
$(LocalResource("img/n_queens.png"))
"""

# ╔═╡ e027e443-218b-46be-9418-e8bf0cb99366


# ╔═╡ 1422bda2-3fea-48d0-ba02-847b122f0761
md"""

# How about nonlinear optimization?

* **Example 3**   [Rosenbrock](https://en.wikipedia.org/wiki/Rosenbrock_function) functions
...
* Let us use `JuMP` and `Ipopt` to minimze  Rosenbrock function 
```math
...
# ╔═╡ c5e8abed-7bf3-4663-8621-827852660903
let
	f(x) = (1-x[1])^2 + 100(x[2]-x[1]^2)^2
    x_r = range(-1.5, 1.5, length=400)
    y_r = range(-0.5 , 1.5, length=400)
    contour(x_r,y_r,(x,y) -> f([x;y]), levels = 0.1:5.0:500)
    scatter!([1.0],[1.0],c=:red,label=:false)
end

# ╔═╡ d1dae6b4-9d9c-4c9f-b3f0-e64ab8b4f897
md"""
##
...
# ╔═╡ b39dbd11-1691-4ddb-a4ca-f4d5bdb1c701
begin
    rosen = Model(Ipopt.Optimizer)
    
    @variable(rosen, x_rosen[1:2])
    
    @NLobjective(rosen, Min, (1 - x_rosen[1])^2 + 100(x_rosen[2] - x_rosen[1]^2)^2)
...
# ╔═╡ 02f021c7-d2ee-483d-af60-8e6fbdaf1b95
value.(x_rosen)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "e9ffada6f02160f5d1cb610e30fd2549a122ce76"
